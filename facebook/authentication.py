...
def deauthenticate():
    cherrypy.response.cookie[constants.FACEBOOK_ACCESS_TOKEN] = cherrypy.response.cookie[constants.FACEBOOK_ACCESS_TOKEN]
...
def authenticate():
    raise cherrypy.HTTPRedirect("https://graph.facebook.com/oauth/access_token?" \
        ...
        "client_secret=" + constants.APP_SECRET + "&code=" + cherrypy.request.cookie[constants.FACEBOOK_CODE])
